...
class Spritesheet:
    """Utility class to load animation sequences from a sprite sheet."""
    ...
    def get_images_at(self, start, end, flip=False) -> list:
        ...
        for i in range(start, end + 1):
            if flip:
                images.append(pg.transform.flip(
                    self.get_image_at_position(i), True, False))
            else:
                images.append(self.get_image_at_position(i))
        return images

    def scale_images_to_size(self, image_width, image_height):
        """Scales the entire spritesheet such that the width
        and height of each image from get_images_at()
